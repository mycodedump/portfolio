    ...
    <AnimatePresence>
      {post && (
        <>
          ...
          <motion.div
            key="drawer"
            initial={{ x: isMobile ? "100%" : 740 }}
            animate={{ x: 0 }}
            exit={{ x: isMobile ? "100%" : 740 }}
          ...
          >
            {/* Close button — desktop only (mobile has it inline in the sticky header) */}
            {!isMobile && (
              <button
                onClick={onClose}
                style={{
            ...
            >
              ...
              <div
                style={{
                  ...
                  backgroundColor: "#e3d9ce",
                  padding: isMobile ? "16px 16px 12px" : scrolled ? "14px 56px 14px 36px" : "36px 56px 20px 36px",
              ...
              >
                {isMobile ? (
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    <p className="font-caslon not-italic" style={{ flex: 1, fontSize: 16, lineHeight: "normal", color: "#212012", fontWeight: 600 }}>
                ...
                {isMobile && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                    <span style={{ backgroundColor: "rgba(221,161,174,0.28)", borderRadius: 20, padding: "3px 10px" }}>
                      <p className="font-jakarta font-medium" style={{ fontSize: 9, color: "#a06070", letterSpacing: "0.3px", textTransform: "uppercase" }}>
                        {post.category}
                      </p>
                    </span>
                    <p className="font-jakarta" style={{ fontSize: 11, color: "#625e37", opacity: 0.6 }}>{post.date}</p>
                    <span style={{ color: "rgba(98,94,55,0.3)", fontSize: 10 }}>·</span>
                    <p className="font-jakarta" style={{ fontSize: 11, color: "#625e37", opacity: 0.5 }}>{post.readTime}</p>
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: isMobile ? "20px 16px 32px" : "28px 52px 40px 36px" }}>
                <p className="font-jakarta" style={{ fontSize: isMobile ? 14 : 16, lineHeight: isMobile ? "21px" : "25px", color: "#625e37", opacity: 0.9, marginBottom: 20 }}>
                  {post.subtitle}
                </p>
                <BlogAudioPlayer slug={post.slug} currentTimeRef={setAudioTime} />
                <div style={{ height: 1, backgroundColor: "rgba(33,32,18,0.1)", marginBottom: isMobile ? 24 : 32 }} />
                <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 18 : 24 }}>
                  {post.body.map((para, i) => (
                    <motion.p
                      ...
                      className="font-caslon not-italic"
                      style={{ fontSize: isMobile ? 15 : 17, lineHeight: isMobile ? "25px" : "30px", color: "#212012", opacity: 0.85 }}
                    >
                      {typeof para === "string" ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: isMobile ? 32 : 48, paddingTop: 24, borderTop: "1px solid rgba(33,32,18,0.08)" }}>
              ...
              {/* Related */}
              <div style={{ padding: isMobile ? "24px 16px 40px" : "32px 52px 56px 36px", borderTop: "1px solid rgba(33,32,18,0.1)" }}>
